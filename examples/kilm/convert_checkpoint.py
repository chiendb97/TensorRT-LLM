...
def load_from_gptq_kilm(
    ...
    for l in tqdm(layers_range, desc="loading weight in each layer..."):
        ...
        dense_bias = model_params[prefix + "attn.c_proj.bias"].to(
            torch_dtype).cpu().contiguous()
        weights[tllm_prex + ".attention.dense.bias"] = dense_bias
...
def convert_hf_kilm(hf_model,
    ...
    intermediate_size = hf_model.config.intermediate_size // 2  # KiLM's actual intermediate_size is one half of what's in hf_config
    ...
    for l in layers_range:
        ...
        attn_dense_weight, attn_dense_bias = get_weight_and_bias(model_params, prefix + 'attn.c_proj',
                                                                 dtype)
...
def main():
    ...
    if args.model_dir is not None:
        ...
        if args.smoothquant is not None or args.int8_kv_cache:
            ...
            if args.smoothquant is not None:
                smooth_kilm_model(model, act_range, args.smoothquant,
                                  kilm_qkv_para, kilm_smoother)
    convert_args = {
        'hf_model': model,
        'act_range': act_range,
    ...
    def covert_and_save(rank, convert_args):
        ...
        if args.use_weight_only and args.weight_only_precision == 'int4_gptq':
            weights = load_from_gptq_kilm(convert_args['hf_model'],
                                          args.n_layer,
                                          mapping,
                                          dtype=args.dtype)
            ...
            weights = convert_hf_kilm(
                ...
                qkv_para=convert_args['kilm_qkv_para'],
                smoother=convert_args['kilm_smoother'])
