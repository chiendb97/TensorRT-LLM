...
This document shows how to build and run a KiLM model in TensorRT-LLM on both single GPU, single node multi-GPU and multi-node multi-GPU.

## Overview

The TensorRT-LLM KiLM implementation can be found in [model.py](model.py). The TensorRT-LLM KiLM example code is located in [`examples/kilm`](./). There is one main file:

* [`build.py`](./build.py) to build the [TensorRT](https://developer.nvidia.com/tensorrt) engine(s) needed to run the KiLM model.
...
Need to prepare the HF KiLM checkpoint first by following the guides here [KiLM-7B-Chat](https://huggingface.co/KiLM/KiLM-7B-Chat) or [KiLM-14B-Chat](https://huggingface.co/KiLM/KiLM-14B-Chat)

Create a `tmp/KiLM` directory to store the weights downloaded from huaggingface.
```bash
mkdir -p ./tmp/KiLM
```

Store KiLM-7B-Chat or KiLM-14B-Chat separately.
- for KiLM-7B-Chat
```bash
mv KiLM-7B-Chat ./tmp/KiLM/7B
```
- for KiLM-14B-Chat
```
mv KiLM-14B-Chat ./tmp/KiLM/14B
```

TensorRT-LLM KiLM builds TensorRT engine(s) from HF checkpoint. If no checkpoint directory is specified, TensorRT-LLM will build engine(s) with dummy weights.
...
# use_gpt_attention_plugin is necessary in KiLM.
# Try use_gemm_plugin to prevent accuracy issue.
# It is recommend to use --remove_input_padding along with --use_gpt_attention_plugin for better performance

# Build the KiLM 7B model using a single GPU and FP16.
python build.py --hf_model_dir ./tmp/KiLM/7B/ \
                --dtype float16 \
...
# Build the KiLM 7B model using a single GPU and BF16.
python build.py --hf_model_dir ./tmp/KiLM/7B/ \
                --dtype bfloat16 \
                --remove_input_padding \
...
# Build the KiLM 7B model using a single GPU and apply INT8 weight-only quantization.
python build.py --hf_model_dir ./tmp/KiLM/7B/ \
                --dtype float16 \
                --remove_input_padding \
                --use_gpt_attention_plugin float16 \
                ...
                --output_dir ./tmp/KiLM/7B/trt_engines/int8_weight_only/1-gpu/

# Build the KiLM 7B model using a single GPU and apply INT4 weight-only quantization.
python build.py --hf_model_dir ./tmp/KiLM/7B/ \
                --dtype float16 \
                --remove_input_padding \
                --use_gpt_attention_plugin float16 \
                ...
                --output_dir ./tmp/KiLM/7B/trt_engines/int4_weight_only/1-gpu/

# Build KiLM 7B using 2-way tensor parallelism.
python build.py --hf_model_dir ./tmp/KiLM/7B/ \
                --dtype float16 \
                --remove_input_padding \
                --use_gpt_attention_plugin float16 \
...
# Build KiLM 7B using 2-way tensor parallelism and 2-way pipeline parallelism.
python build.py --hf_model_dir ./tmp/KiLM/7B/ \
                --dtype float16 \
                --remove_input_padding \
                --use_gpt_attention_plugin float16 \
...
# Build KiLM 14B using 2-way tensor parallelism.
python build.py --hf_model_dir ./tmp/KiLM/14B \
                --dtype float16 \
                --remove_input_padding \
                --use_gpt_attention_plugin float16 \
                --enable_context_fmha \
...
```python
python3 build.py --hf_model_dir /llm-models/KiLM-7B-Chat/ --output_dir /engine_kilm
```
```
...
[11/09/2023-00:57:14] [TRT-LLM] [I] Loading HF KiLM ... from /llm-models/KiLM-7B-Chat/
......
[11/09/2023-01:01:34] [TRT] [I] [MemUsageStats] Peak memory usage during Engine building and serialization: CPU: 47322 MiB
[11/09/2023-01:01:34] [TRT-LLM] [I] Total time of building kilm_float16_tp1_rank0.engine: 00:03:44
[11/09/2023-01:01:34] [TRT-LLM] [I] Config saved to /engine_kilm/config.json.
[11/09/2023-01:01:34] [TRT-LLM] [I] Serializing engine to /engine_kilm/kilm_float16_tp1_rank0.engine...
[11/09/2023-01:01:49] [TRT-LLM] [I] Engine serialized. Total time: 00:00:14
[11/09/2023-01:01:49] [TRT-LLM] [I] Timing cache serialized to /engine_kilm/model.cache
[11/09/2023-01:01:50] [TRT-LLM] [I] Total time of building all 1 engines: 00:04:43
```


#### INT8 weight only + INT8 KV cache
For INT8 KV cache, [`hf_kilm_convert.py`](./hf_kilm_convert.py) features a
...
```bash
python3 hf_kilm_convert.py \
    -i ./tmp/KiLM/7B/ \
    -o ./tmp/KiLM/7B/int8_kv_cache/ \
...
python build.py --ft_dir_path ./tmp/KiLM/7B/int8_kv_cache/1-gpu/ \
                --dtype float16 \
                --hf_model_dir ./tmp/KiLM/7B \
                --use_gpt_attention_plugin float16 \
                --use_gemm_plugin float16 \
                --output_dir ./tmp/KiLM/7B/trt_engines/int8_kv_cache_weight_only/1-gpu \
                --int8_kv_cache \
                --use_weight_only
```
...
python3 ../run.py --input_text "你好，请问你叫什么？" \
                  ...
                  --engine_dir=./tmp/KiLM/7B/trt_engines/int8_kv_cache_weight_only/1-gpu
```
...
The smoothquant supports both KiLM v1 and KiLM v2. Unlike the FP16 build where the HF weights are processed and loaded into the TensorRT-LLM directly, the SmoothQuant needs to load INT8 weights which should be pre-processed before building an engine.

Example:
```bash
python3 hf_kilm_convert.py -i ./tmp/KiLM/7B -o ./tmp/KiLM/7B/sq0.5/ -sq 0.5 --tensor-parallelism 1 --storage-type float16
```
...
python3 build.py --ft_dir_path=./tmp/KiLM/7B/sq0.5/1-gpu/ \
                 --use_gpt_attention_plugin float16 \
                 --remove_input_padding \
                 --enable_context_fmha \
                 --use_smooth_quant \
                 ...
                 --per_channel \
                 --hf_model_dir ./tmp/KiLM/7B \
                 --output_dir ./tmp/KiLM/7B/trt_engines/sq0.5/1-gpu/
```
...
python3 ../run.py --input_text "你好，请问你叫什么？" \
                  --max_output_len=50 \
                  --tokenizer_dir ./tmp/KiLM/7B/ \
                  --engine_dir=./tmp/KiLM/7B/trt_engines/sq0.5/1-gpu/
```
...
```
#### INT4-GPTQ
To run the GPTQ KiLM example, the following steps are required:
...
```bash
python3 gptq_convert.py --hf_model_dir ./tmp/KiLM/7B \
                        --tokenizer_dir ./tmp/KiLM/7B \
                        --quant_ckpt_path ./tmp/KiLM/7B/int4-gptq
```

3. Build TRT-LLM engine:
```bash
python build.py --hf_model_dir ./tmp/KiLM/7B \
                --quant_ckpt_path ./tmp/KiLM/7B/int4-gptq/gptq_model-4bit-128g.safetensors \
                --dtype float16 \
                --remove_input_padding \
                --use_gpt_attention_plugin float16 \
                --enable_context_fmha \
                ...
                --world_size 1 \
                --tp_size 1 \
                --output_dir ./tmp/KiLM/7B/trt_engines/int4-gptq/1-gpu
```
...
python3 ../run.py --input_text "你好，请问你叫什么？" \
                  --max_output_len=50 \
                  --tokenizer_dir ./tmp/KiLM/7B/ \
                  --engine_dir=./tmp/KiLM/7B/trt_engines/int4-gptq/1-gpu
```
```
......
...
```bash
python3 quantize.py --model_dir ./tmp/KiLM/7B \
                    --dtype float16 \
                    --qformat int4_awq \
                    --export_path ./kilm_7b_4bit_gs128_awq.pt \
                    --calib_size 32
```
...
```bash
python build.py --hf_model_dir ./tmp/KiLM/7B \
                --quant_ckpt_path ./kilm_7b_4bit_gs128_awq.pt \
                --dtype float16 \
                --remove_input_padding \
                --use_gpt_attention_plugin float16 \
                --enable_context_fmha \
                ...
                --world_size 1 \
                --tp_size 1 \
                --output_dir ./tmp/KiLM/7B/trt_engines/int4-awq/1-gpu
```
3. Run int4-awq
```bash
python3 ../run.py --input_text "你好，请问你叫什么？" \
                  --max_output_len=50 \
                  --tokenizer_dir ./tmp/KiLM/7B/ \
                  --engine_dir=./tmp/KiLM/7B/trt_engines/int4-awq/1-gpu
```
```
......
Input [Text 0]: "<|im_start|>system
...
To run a TensorRT-LLM KiLM model using the engines generated by build.py
...
python3 ../run.py --input_text "你好，请问你叫什么？" \
                  --max_output_len=50 \
                  --tokenizer_dir /llm-models/KiLM-7B-Chat/ \
                  --engine_dir /engine_kilm
```

```
Loading engine from /engine_kilm/kilm_float16_tp1_rank0.engine
...
```python
python3 ../summarize.py --test_trt_llm --tokenizer_dir /llm-models/KiLM-7B-Chat/ --engine_dir /engine_kilm --max_input_length 2048 --output_len 2048
```
```
