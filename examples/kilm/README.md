...
The TensorRT-LLM Qwen implementation can be found in [model.py](../../tensorrt_llm/models/kilm/model.py). The TensorRT-LLM Qwen example code is located in [`examples/kilm`](./). There is one main file:
...
The TensorRT-LLM Qwen example code locates at [examples/kilm](./). It takes HF weights as input, and builds the corresponding TensorRT engines. The number of TensorRT engines depends on the number of GPUs used to run inference.
...
```python
python3 build.py --hf_model_dir /llm-models/Qwen-7B-Chat/ --output_dir /engine_kilm
```
```
...
[11/09/2023-00:57:14] [TRT-LLM] [I] Loading HF KiLM ... from /llm-models/Qwen-7B-Chat/
......
...
Example:
```bash
python3 hf_kilm_convert.py -i ./tmp/Qwen/7B -o ./tmp/Qwen/7B/sq0.5/ -sq 0.5 --tensor-parallelism 1 --storage-type float16
```
...
```python
python3 ../summarize.py --test_trt_llm --tokenizer_dir /llm-models/Qwen-7B-Chat/ --engine_dir /engine_kilm --max_input_length 2048 --output_len 2048
```
```
