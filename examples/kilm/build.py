...
from weight import (load_from_awq_kilm, load_from_ft, load_from_gptq_kilm,
                    load_from_hf_kilm)
...
def parse_arguments():
    ...
    if args.hf_model_dir is not None:
        ...
        args.inter_size = hf_config.intermediate_size  # override the inter_size for KiLM
    ...
    assert args.use_gpt_attention_plugin is not None, "KiLM must use gpt attention plugin"
...
def build_rank_engine(builder: Builder,
    ...
    # Initialize Module
    tensorrt_llm_kilm = tensorrt_llm.models.KiLMForCausalLM(
    ...
    tensorrt_llm_kilm = quantize_model(tensorrt_llm_kilm, args.quant_mode,
                                       **quantize_kwargs)
    ft_dir_path = args.ft_dir_path
    if args.per_group:
        load_func = load_from_awq_kilm if args.weight_only_precision == 'int4_awq' else load_from_gptq_kilm
        load_func(tensorrt_llm_kilm=tensorrt_llm_kilm,
        ...
        logger.info(f'Loading HF KiLM ... from {args.hf_model_dir}')
        tik = time.time()
        hf_kilm = AutoModelForCausalLM.from_pretrained(
            args.hf_model_dir,
            device_map={
                "transformer": "cpu",
                "lm_head": "cpu"
        ...
        tok = time.time()
        t = time.strftime('%H:%M:%S', time.gmtime(tok - tik))
        logger.info(f'HF KiLM loaded. Total time: {t}')
        load_from_hf_kilm(tensorrt_llm_kilm,
                          hf_kilm,
                          mapping,
                          max_position_embeddings=args.n_positions,
        ...
        logger.info(f'Loading FT KiLM ... from {ft_dir_path}')
        load_from_ft(tensorrt_llm_kilm,
                     dir_path,
                     mapping,
                     dtype=args.dtype,
                     multi_query_mode=multi_query_mode)
    ...
    with net_guard(network):
        # Prepare
        network.set_named_parameters(tensorrt_llm_kilm.named_parameters())
        ...
        if args.enable_debug_output:
            ...
            for k, v in tensorrt_llm_kilm.named_network_outputs():
                v = v.trt_tensor
                v.name = k
