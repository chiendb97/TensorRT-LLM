...
from tensorrt_llm.models import KiLMForCausalLM
...
def parse_ft_config(ini_file):
    ...
    vocab_size = kilm_config.getint('kilm', 'vocab_size')
    hidden_size = kilm_config.getint('kilm', 'hidden_size')
    inter_size = kilm_config.getint('kilm', 'intermediate_size', fallback=None)
    num_hidden_layers = kilm_config.getint(
        "kilm",
        "num_hidden_layers",
        fallback=32,
    )
    max_position_embeddings = kilm_config.getint("kilm",
                                                 "max_position_embeddings",
                                                 fallback=8192)
    kv_channels = kilm_config.getint('kilm', 'kv_channels', fallback=128)
    rotary_pct = kilm_config.getfloat('kilm', 'rotary_pct', fallback=0.0)
    rotary_emb_base = kilm_config.getint('kilm',
                                         'rotary_emb_base',
                                         fallback=10000)
    multi_query_mode = kilm_config.getboolean('kilm',
                                              'multi_query_mode',
                                              fallback=False)
...
def load_from_ft(tensorrt_llm_kilm: KiLMForCausalLM,
    ...
    if mapping.is_last_pp_rank():
        tensorrt_llm_kilm.ln_f.weight.value = (fromfile(dir_path,
                                                        'ln_f.weight.bin'))
    ...
    for i in layers_range:
        ...
        t = fromfile(
            dir_path, 'model.layers.' + str(i) + '.attention.dense.bias.bin', [hidden_size])
        dst.value = np.ascontiguousarray(t)
        ...
        if use_int8_kv_cache:
            ...
            tensorrt_llm_kilm.layers[i].attention.kv_quant_orig_scale.value = t
...
def load_from_gptq_kilm(
        tensorrt_llm_kilm: KiLMForCausalLM,
        quant_ckpt_path,
        mapping=Mapping(),
        dtype="float16",
):
    tensorrt_llm.logger.info(
        "loading weights from groupwise gptq kilm safetensors...")
    tik = time.time()
    ...
    for layer in tqdm(layers_range,
        ...
        tensorrt_llm_kilm.layers[idx].attention.qkv.zero.value = th_zero.to(
            torch_dtype).numpy()
        tensorrt_llm_kilm.layers[
    ...
    for k, v in tqdm(model_params.items(),
        ...
        if "transformer.wte.weight" in k:
            ...
            if "ln_1.weight" in k:
                ...
                tensorrt_llm_kilm.layers[idx].mlp.gate.zero.value = th_zero.to(
                    torch_dtype).numpy()
                tensorrt_llm_kilm.layers[
                ...
                tensorrt_llm_kilm.layers[idx].mlp.proj.zero.value = th_zero.to(
                    torch_dtype).numpy()
                tensorrt_llm_kilm.layers[
                ...
                tensorrt_llm_kilm.layers[idx].mlp.fc.zero.value = th_zero.to(
                    torch_dtype).numpy()
                tensorrt_llm_kilm.layers[
    ...
    for layer_idx in tqdm(layers_range, "Loading weights..."):
        ...
        for idx, awq_attr in enumerate(awq_block_names):
            ...
            layer = attrgetter(tensorrt_llm_block_names[idx])(
                tensorrt_llm_kilm.layers[layer_idx])
            setattr(layer, 'value', v.to(torch_dtype).cpu().numpy())

        mPrefix = prefix + "attn.c_attn"
        mOp = tensorrt_llm_kilm.layers[layer_idx].attention.qkv
        ...
        th_bias = model_params[prefix + "attn.c_proj.bias"].cpu().to(
            torch_dtype).contiguous()
        tensorrt_llm_kilm.layers[
            layer_idx].attention.dense.bias.value = np.ascontiguousarray(th_bias)

        # MLP up_proj (mlp.gate) Linear
        mPrefix = prefix + "mlp.w2"
        mOp = tensorrt_llm_kilm.layers[layer_idx].mlp.fc
        process_and_assign_weight(model_params, mPrefix, mOp, 1)

        # MLP down_proj (mlp.proj) Linear
        mPrefix = prefix + "mlp.w1"
        mOp = tensorrt_llm_kilm.layers[layer_idx].mlp.gate
        process_and_assign_weight(model_params, mPrefix, mOp, 0)

        # MLP gate_proj (mlp.fc) Linear
        mPrefix = prefix + "mlp.c_proj"
        mOp = tensorrt_llm_kilm.layers[layer_idx].mlp.proj
        process_and_assign_weight(model_params, mPrefix, mOp, 1)
