...
def get_all_lora_weights(lora_weights):
    ...
    pattern = re.compile(
        r'.*\.transformer\.h\.([0-9]+)\.(attn|mlp)\.([a-z1-2_]+)\.lora_(A|B)\.weight.*'
    )
...
def convert_hf_model(model_dir, dtype, out_dir):
    ...
    if os.path.isfile(f"{model_dir}/adapter_model.bin"):
        lora_model = torch.load(f"{model_dir}/adapter_model.bin")
    else:
        lora_model = load_file(f"{model_dir}/adapter_model.safetensors")
