...
from tensorrt_llm.models import InternVisionModel
...
def convert_and_save_hf(args):
    ...
    def convert_and_save_rank(args, rank, tllm_class: Union[
        ...
        tik = time.time()
        tllm_intern_vision = tllm_class.from_hugging_face(
            model_dir,
            args.dtype,
            mapping=mapping,
        ...
        tik = time.time()
        tllm_intern_vision.save_checkpoint(args.output_dir, save_config=(rank == 0))
        del tllm_intern_vision
...
def execute(workers, func, args,
            tllm_class: Union[InternVisionModel]):
    if workers == 1:
