...
if [ -n "${GITHUB_MIRROR}" ]; then
  BOOST_URL="https://urm.nvidia.com/artifactory/sw-dl-triton-generic-local/triton/ci-cd/binaries/boost/1.86.0/boost_1_86_0.tar.gz"
else
...
install_boost() {
  ...
  wget --no-verbose --retry-connrefused --timeout=180 --tries=10 --continue -O /tmp/boost.tar.gz ${BOOST_URL} \
    ...
    && mv /tmp/boost_1_86_0/boost /usr/include/boost \
