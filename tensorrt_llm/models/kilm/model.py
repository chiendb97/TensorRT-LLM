...
class KiLMAttention(Module):
    ...
    def forward(
        ...
        if not default_net().plugin_config.gpt_attention_plugin:
            raise ValueError('KiLM is only supported with GPTAttention plugin')
...
class KiLMForCausalLM(KiLMModel, GenerationMixin):
