...
from transformers.models.kilm2.modeling_kilm2 import KiLM2DecoderLayer
...
from .utils import get_kilm_key_list, make_context
from .weight import load_from_gptq_kilm
...
@torch.no_grad()
def smooth_kilm_model(model, scales, alpha, kilm_qkv_para, kilm_smoother):
    # Smooth the activation and weights with smoother = $\diag{s}$
    for name, module in model.named_modules():
        if not module._get_name() == "KiLMBlock":
            continue
        # qkv_proj
        ...
        kilm_qkv_para[layer_name] = module.attn.c_attn.weight.transpose(0, 1)
        ...
        kilm_smoother[layer_name] = smoother.float()
...
@torch.no_grad()
def smooth_kilm2_model(model, scales, alpha, kilm_qkv_para, kilm_smoother):
    # Smooth the activation and weights with smoother = $\diag{s}$
    for name, module in model.named_modules():
        if not isinstance(module, KiLM2DecoderLayer):
            continue
        # qkv_proj
        ...
        # see transpose_weights function
        kilm_qkv_para[layer_name_qkv] = weight.transpose(0, 1)
...
def convert_hf_kilm(hf_model,
    ...
    if kilm_type == 'kilm':
        intermediate_size = hf_model.config.intermediate_size // 2  # KiLM version 1 has actual intermediate_size one half of what's in hf_config
    else:
    ...
    for l in layers_range:
        ...
        if use_smooth_quant:
            qkv_proj_key = key_list[
                0] if kilm_type == 'kilm' else 'self_attn.qkv_proj'
        ...
        attn_dense_weight, attn_dense_bias = get_weight_and_bias(model_params, prefix + key_list[1],
                                                                 dtype)
        ...
        if kilm_type == "kilm2_moe" and moe_config and moe_config.has_moe():

            # shared_expert for kilm2_moe
...
def smooth_quant(model,
    ...
    if smoothquant is not None:
        if kilm_type == 'kilm':
            smooth_kilm_model(model, act_range, smoothquant, kilm_qkv_para,
                              kilm_smoother)
        else:
            smooth_kilm2_model(model, act_range, smoothquant, kilm_qkv_para,
                               kilm_smoother)
    return act_range, kilm_qkv_para, kilm_smoother
...
def create_config_from_hugging_face(hf_model,
    ...
    if kilm_type == "kilm2_moe":
        hidden_act = "swiglu"

    config.update({
        'architecture': "KiLMForCausalLM",
        'dtype': dtype,
...
def from_hugging_face(cls,
                      ...
                      *,
                      ...
                      from_hf_gptq=False,
                      override_fields={}):
    ''' Create a KiLMForCausalLM object from give parameters
    '''
    ...
    kilm_type = pretrained_config.kilm_type
    assert kilm_type in ['kilm', 'kilm2', 'kilm2_moe'], "Unsupported KiLM type."
    kilm = cls.from_config(pretrained_config)

    if from_hf_gptq:
        weights = load_from_gptq_kilm(
...
def quantize(dtype,
    ...
    act_range, kilm_qkv_para, kilm_smoother = smooth_quant(
        model, kilm_type, model_dir, calib_dataset, dataset_cache_dir,
        smoothquant_val)
    ...
    for rank in range(mapping.world_size):
        ...
        weights = load_weights_from_hf(
            ...
            act_range=act_range,
            kilm_qkv_para=kilm_qkv_para,
            kilm_smoother=kilm_smoother)
...
def load_weights_from_hf(*,
    ...
    weights = convert_hf_kilm(
        ...
        act_range=act_range,
        qkv_para=kilm_qkv_para,
        smoother=kilm_smoother,
